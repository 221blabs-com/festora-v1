...
class ExternalForce extends ShaderPass {
  mouseInput: MouseClass;
  mouse!: THREE.Mesh;

  constructor(common: CommonClass, mouseInput: MouseClass, simProps: any) {
  ...
  update(...args: any[]) {
    const props = args[0] || {};
    const forceX = (this.mouseInput.diff.x / 2) * (props.mouse_force || 0);
    const forceY = (this.mouseInput.diff.y / 2) * (props.mouse_force || 0);
    ...
    const centerX = Math.min(
      Math.max(this.mouseInput.coords.x, -1 + cursorSizeX + cellScale.x * 2),
      1 - cursorSizeX - cellScale.x * 2
    );
    const centerY = Math.min(
      Math.max(this.mouseInput.coords.y, -1 + cursorSizeY + cellScale.y * 2),
...
class Output {
  ...
  constructor(common: CommonClass, mouse: MouseClass, paletteTex: THREE.DataTexture) {
    this.common = common;
...
export class WebGLManager implements LiquidEtherWebGL {
  common: CommonClass;
  mouse: MouseClass;
  props: any;
  output!: Output;
  ...
  running = false;
  private _loop = this.loopInternal.bind(this);
  private _resize = this.resize.bind(this);
  private _onVisibility?: () => void;
  // External ref to isVisible to check in loop/visibility handler if needed
  // But logic handles it via start/pause

  constructor(props: any, paletteTex: THREE.DataTexture) {
    this.props = props;
    ...
    this.init(paletteTex);
    window.addEventListener('resize', this._resize);
    // Visibility listener added in component to control start/stop via references
  }

  init(paletteTex: THREE.DataTexture) {
    if (!this.common.renderer) return;
    this.props.$wrapper.prepend(this.common.renderer.domElement);
    this.output = new Output(this.common, this.mouse, paletteTex);
  }
  ...
  loopInternal() {
     if (!this.running) return;
     this.render();
     this.rafId = requestAnimationFrame(() => this.loopInternal());
  }
